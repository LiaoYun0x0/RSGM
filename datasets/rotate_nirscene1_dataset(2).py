...
class RotateNIRDataset(Dataset):
    ...
    def _generate_ref1(self, refer, query):
        """
    ...
    def _generate_ref(self, refer, query):
        ...
        # --- 训练分支 (self.aug is True) ---
        # (有 2/3 的概率进入这里)
        if random.sample([0, 1, 2], 1)[0] != 0 and self.aug is True:
            # 1. 随机裁剪
            crop_query, crop_M_query, qc = self._random_crop(query)
            # 2. 随机增强 (旋转, 翻转等, 因为 aug=1 (默认))
            query, Mq_ = self._aug_img(crop_query, query, qc)  # 320x320x3, 3x3
            Mq = np.matmul(Mq_, crop_M_query)  # 转换矩阵= 裁剪+增强
        ...
        # --- 测试分支 (self.aug is False) ---
        # (或者 1/3 的训练概率)
        else:
            # 1. 【修改】使用 _random_crop 进行随机裁剪
            crop_query, crop_M_query, qc = self._random_crop(query)  # <-- 修改
            # 2. 【保持】调用 _aug_img(aug=-1) 来禁用旋转/翻转
            query, Mq = self._aug_img(crop_query, query, qc, aug=-1)
            Mq = np.matmul(Mq, crop_M_query)

            # 1. 【修改】对 refer 使用 _random_crop 进行随机裁剪
            crop_refer, crop_M_refer, rc = self._random_crop(refer)  # <-- 修改

            # 2. 【删除】移除固定的 padding
            # crop_refer = cv2.copyMakeBorder(...) # <-- 删除此行

            # 3. 【保持】调用 _aug_img(aug=-1)
            refer, Mr = self._aug_img(crop_refer, refer, rc, aug=-1)
            Mr = np.matmul(Mr, crop_M_refer)

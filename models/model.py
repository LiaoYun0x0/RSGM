...
from models.loftr_module.transformer import LocalFeatureTransformer as LocalTransformer, Relator_Fusion, FeatureFusion, \
    TopicFormer
from models.position import PositionEmbedding2D, PositionEmbedding1D
from models.resnet.swin_transformer import swin_base_patch4_window7_224
from models.resnet_fpn import ResNetFPN_8_2
from models.transformer import LocalFeatureTransformer, GlobalFeatureTransformer, \
    PositionEncodingSine, LocalFineFeatureTransformer, LocalFeatureTransformer
...
class MatchingNet(nn.Module):
    def __init__(
        ...
        self.backbone_2 = ResNetFPN_8_2(config['resnetfpn'])
        ...
        # self.local_transformer = LocalFeatureTransformer(cfg["lo_cfg"])
        # self.loftr_coarse = LocalTransformer(config['coarse'])
        self.relator_fusion = Relator_Fusion(config['coarse'])
        self.feature_fusion = FeatureFusion(config['coarse'])
    ...
    def forward(self, samples0, samples1, gt_matrix):
        ...
        # print(feats_c.size())
        # print(feats_f.size())
        # (topic0_f, topic1_f), (feat_c0_f, feat_c1_f), (feat_f0_f, feat_f1_f) = topic_2.split(1), feats_c_2.split(
        #     1), feats_f_2.split(1)
        # 1x1600x256, 1x256x160x160
        # mdesc0, mdesc1, fine_featmap0, fine_featmap1 = self.backbone.forward_pair_lo(samples0, samples1)
        # topic, feats_c, feats_f = self.backbone(torch.cat([data['image0'], data['image1']], dim=0))
        (topic0, topic1), (mdesc0, mdesc1), (fine_featmap0, fine_featmap1) = topic.split(1), feats_c.split(
            1), feats_f.split(1)
        feat_c0 = mdesc0
        feat_c1 = mdesc1
        feat_c0 = rearrange(self.pos_encoding(feat_c0), 'n c h w -> n (h w) c')
        feat_c1 = rearrange(self.pos_encoding(feat_c1), 'n c h w -> n (h w) c')
        # feat_c0_f = rearrange(self.pos_encoding(feat_c0_f), 'n c h w -> n (h w) c')
        # feat_c1_f = rearrange(self.pos_encoding(feat_c1_f), 'n c h w -> n (h w) c')
        mask_c0 = mask_c1 = None  # mask is useful in training
        if 'mask0' in data:
            mask_c0, mask_c1 = data['mask0'].flatten(-2), data['mask1'].flatten(-2)
        # feat_c0_f, feat_c1_f = self.loftr_coarse(feat_c0_f, feat_c1_f, mask_c0, mask_c1)

        # 3. match coarse-level
        feat_c00, feat_c10 = self.feature_fusion(feat_c0, feat_c1, data)
        # self.feature_fusion(feat_c0, feat_c1, data)
        feat_c01, feat_c11 = self.relator_fusion(feat_c0, feat_c1, data)
        feat_c00 = feat_c0 * 0.5 + feat_c00 * 0.25 + feat_c01 * 0.25
        feat_c10 = feat_c1 * 0.5 + feat_c10 * 0.25 + feat_c11 * 0.25
        feat_c00, feat_c10, topic_matrix, topic_matrix_match = self.coarse_net(feat_c00, feat_c10, topic0,
                                                                               topic1, mask_c0,
                                                                               mask_c1)
        self.coarse_matching(feat_c0, feat_c1, feat_c00, feat_c10, topic_matrix_match, data, mask_c0=mask_c0,
                             mask_c1=mask_c1)
        # cm_matrix = self.compute_confidence_matrix(mdesc0, mdesc1)
        cm_matrix = data['conf_matrix']
        mdesc0 = (feat_c0 + feat_c00) / 2
        mdesc1 = (feat_c1 + feat_c10) / 2
